...
def printMaskedImage(image, contour):
    
    """  1- Initialize new_image """
    print ('masked_imageeeee  ' , np.array(image).shape)

    """ 2- Round contour Values """
    new_contour = updateContour(contour)
   
    """ 3- Extract x and y of points separatly """
    x,y = getCoordinates(new_contour)
    
    """ 4- Extract masked image only"""
    image, mask, masked, trimmed = extractMaskedImage(x,y,image)
    print ('masked shapeeee' , np.array(masked).shape)
    
    """ Save Mask """
    path = 'mask.png' #full path + image name+extenstion
    saveMask(trimmed, path)
    
    #visualizemaskedImage(masked)
    #visualizeMaskedTrimmed(image, mask, masked, trimmed)
    
def updateContour(contour):
    """ 
    old contour has float indeces
    """
    new_contour = []
...
def getCoordinates(new_contour):
...
def extractMaskedImage (x,y , masked_image):
    ...
    mask         = mask.reshape(masked_image.shape)
    
    # create a masked array
    masked       = np.ma.masked_array(masked_image, ~mask)
    ...
    return masked_image, mask, masked, trimmed
...
""" ************************************************************************************************** """    
def visualizemaskedImage(image, axx=None):
    ...
    axx.imshow(image.astype(np.uint8))
    if visualize:
        plt.show()  
...
def printMaskZeros(masked_image, contour):
    """ """
    print ('masked_image  ' , np.array(masked_image).shape)
    
    #new_image = np.zeros((np.array(masked_image).shape))
    new_image = masked_image #.shape[0], masked_image.shape[1],3))
    ...
    visualizeImage(new_image, contour)
